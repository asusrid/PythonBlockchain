...
def test_transaction_exceeds_balance():
	with pytest.raises(Exception, match='Amount exceeds the sender balance'):
		Transaction(Wallet(), 'recipient', 1001)


def test_tx_update_exceeds_balance():
	sender_wallet = Wallet()
	transaction = Transaction(sender_wallet, 'recipient', 50)

	with pytest.raises(Exception, match='Amount exceeds the sender balance'):
		transaction.update(sender_wallet, 'new_recipient', 1001)
...
def test_valid_tx_with_invalid_outputs():
	sender_wallet = Wallet()
	transaction = Transaction(sender_wallet, 'recipient', 50)
